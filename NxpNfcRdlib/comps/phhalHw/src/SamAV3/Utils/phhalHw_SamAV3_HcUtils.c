...
/** \file
* Sam AV3 Generic Tools Component.
* $Author: NXP $
* $Revision: $ (v07.10.00)
* $Date: $
*
...
#include "phhalHw_SamAV3_HcUtils.h"
...
phStatus_t phhalHw_SamAV3_Utils_GetCheckLcLe(uint8_t * pCmd, uint16_t wCmdLen, uint8_t * pIsLcPresent, uint8_t * pLcLen, uint8_t * pIsLePresent)
{
    ...
    /* @3*/
    else if (wCmdLen == ((uint16_t)PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH + pCmd[PHHAL_HW_SAMAV3_ISO7816_LC_POS]))
    {
    ...
    /* @4*/
    else if (wCmdLen == ((uint16_t)PHHAL_HW_SAMAV3_ISO7816_HEADER_LE_LENGTH + pCmd[PHHAL_HW_SAMAV3_ISO7816_LC_POS]))
    {
        *pLcLen = pCmd[PHHAL_HW_SAMAV3_ISO7816_LC_POS];
        *pIsLcPresent = PH_ON;
        *pIsLePresent = PH_ON;
...
phStatus_t phhalHw_SamAV3_Utils_ResolveErrorCode(uint8_t * pSw1Sw2)
{
    ...
    switch((phStatus_t) ((((uint16_t)pSw1Sw2[0] << 8) | pSw1Sw2[1]) & 0xFFFF))
    {
	...
	uint8_t bKeyVer, uint8_t * pSessionKey, uint8_t * pKeyType)
{
	...
	uint8_t		PH_MEMLOC_REM aDefault_Iv[PH_CRYPTOSYM_AES128_KEY_SIZE] = {0};
...
phStatus_t phhalHw_SamAV3_Utils_UpdateLc(phhalHw_SamAV3_DataParams_t * pDataParams)
{
    ...
    pTmpBuffer[PHHAL_HW_SAMAV3_ISO7816_LC_POS] = (uint8_t)((pDataParams->wTxBufLen_Cmd - PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH) & 0xFF);

    return PH_ADD_COMPCODE(PH_ERR_SUCCESS, PH_COMP_HAL);
}

phStatus_t phhalHw_SamAV3_Utils_UpdateP1(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bP1)
{
    uint8_t * PH_MEMLOC_REM pTmpBuffer;
...
phStatus_t phhalHw_SamAV3_Utils_UpdateP2(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bP2)
{
    uint8_t * PH_MEMLOC_REM pTmpBuffer;
