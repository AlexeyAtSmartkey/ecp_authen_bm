...
phStatus_t phhalHw_SamAV3_Cmd_7816Exchange(phhalHw_SamAV3_DataParams_t * pDataParams, uint16_t wOption, uint8_t * pTxBuffer, uint16_t wTxLength, uint8_t ** ppRxBuffer,
	uint16_t * pRxLength)
{
    ...
    if (bSamCommand == PHHAL_HW_SAMAV3_CMD_ISO14443_3_TRANSPARENT_EXCHANGE_INS)
    {
        ...
        wTmpBufferSize = ((pDataParams->wTxBufSize - (wTxStartPosTmp + pDataParams->wTxBufLen)) & 0xFFFF);
    }
    ...
    else
    {
        ...
        if (pDataParams->pTxBuffer == pDataParams->pRxBuffer)
        {
                ...
                (pDataParams->wRxBufLen))
            {
                ...
                wTmpBufferSize = ((pDataParams->wTxBufSize - (wTxStartPosTmp + pDataParams->wTxBufLen_Cmd)) & 0xFFFF);
            }
    ...
    pDataParams->pTxBuffer[wTxStartPosTmp + PHHAL_HW_SAMAV3_ISO7816_CLA_POS] |= pDataParams->bLogicalChannel;
    ...
    if (pDataParams->bCmdSM & PHHAL_HW_SAMAV3_HSM_AES_ENC)
    {
		PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_SamAV3_HSM_AES_Encrypt(
            ...
            wTxLengthTmp,
            ((wTmpBufferSize + wTxLengthTmp) & 0xFFFF),
            &wTxLengthTmp,
            bFirstCmd,
            bLast));
    }
        ...
        (bSamCommand == PHHAL_HW_SAMAV3_CMD_SAM_DECIPHER_DATA_INS))
    {
        *ppRxBuffer = &pDataParams->pRxBuffer[pDataParams->wRxBufStartPos];
        wTmpBufferSize = ((pDataParams->wRxBufSize - pDataParams->wRxBufStartPos) & 0xFFFF);
    }
    /* Other commands -> Preserve all buffer data */
    else
    {
        ...
        if (pDataParams->pTxBuffer == pDataParams->pRxBuffer)
        {
            ...
            if (pDataParams->wTxBufLen > pDataParams->wRxBufLen)
            {
                *ppRxBuffer = &pDataParams->pTxBuffer[pDataParams->wTxBufStartPos + pDataParams->wTxBufLen];
                wTmpBufferSize = ((pDataParams->wTxBufSize - (pDataParams->wTxBufStartPos + pDataParams->wTxBufLen)) & 0xFFFF);
            }
            /* Start after RxBuffer contents */
            else
            {
                *ppRxBuffer = &pDataParams->pRxBuffer[pDataParams->wRxBufLen];
                wTmpBufferSize = ((pDataParams->wRxBufSize - pDataParams->wRxBufLen) & 0xFFFF);
            }
        }
        /* Buffers are different */
        else
    ...
    status = PHBALREG_EXCHANGE(
        (phbalReg_Sam_DataParams_t *)pDataParams->pBalDataParams,
        PH_EXCHANGE_DEFAULT,
        &pDataParams->pTxBuffer[wTxStartPosTmp],
        wTxLengthTmp,
        wTmpBufferSize,
    ...
    *pRxLength = *pRxLength - PHHAL_HW_SAMAV3_ISO7816_SW1SW2_LENGTH;
    ...
    case PHHAL_HW_SAMAV3_ERR_DESFIRE_GEN:
        ...
        else
        {
            if (*pRxLength == 2)
            {
                pDataParams->wAdditionalInfo = (((((*ppRxBuffer)[0]) << 8 ) | (*ppRxBuffer)[1]) & 0xFF);
            }
        }
        break;
    default:
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_LockUnlock(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bCmdType, uint16_t wRdKeyNo, uint16_t wRdKeyVer, uint8_t bSamKeyNo,
	uint8_t bSamKeyVer, uint8_t bUnlockKeyNo, uint8_t bUnlockKeyVer, uint32_t dwMaxChainBlocks)
{
	...
	switch (bCmdType)
	{
		...
		case PHHAL_HW_SAMAV3_CMD_SAM_LOCK_UNLOCK_TYPE_ACTIVATE_SAM:
			bLcLen = bLcLen + 3;
			aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_LC_POS + 3] = ((dwMaxChainBlocks) & 0xFF);
			aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_LC_POS + 4] = ((dwMaxChainBlocks >> 8) & 0xFF);
			aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_LC_POS + 5] = ((dwMaxChainBlocks >> 16) & 0xFF);
			break;
	...
	wStatus = phhalHw_SamAV3_Cmd_7816Exchange(
		...
		PH_EXCHANGE_DEFAULT,
		aCmdBuf,
		(uint16_t)(bLcLen + PHHAL_HW_SAMAV3_ISO7816_HEADER_LE_LENGTH),
		&pResponse,
		&wRespLen);
	 ...
	 */
	if((bCmdType == PHHAL_HW_SAMAV3_CMD_SAM_LOCK_UNLOCK_TYPE_LOCK_NO_KEY) || (bCmdType == PHHAL_HW_SAMAV3_CMD_SAM_LOCK_UNLOCK_TYPE_LOCK_KEY))
	{
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_AuthenticateHost(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bHostMode, uint16_t wRdKeyNo, uint16_t wRdKeyV, uint8_t bSamKeyNo,
	uint8_t bSamKeyVer)
{
	phStatus_t	PH_MEMLOC_REM wStatus = 0;
	...
	PH_LOG_HELPER_ADDPARAM_BUFFER(PH_LOG_LOGTYPE_DEBUG, SessKey_Enc_SAM_HOST_log, aSessionEncKey, bKeyLen);
	PH_LOG_HELPER_ADDPARAM_BUFFER(PH_LOG_LOGTYPE_DEBUG, SessKey_Mac_SAM_HOST_log, aSessionMacKey, bKeyLen);
	PH_LOG_HELPER_EXECUTE(PH_LOG_OPTION_CATEGORY_GEN);
...
{
	phStatus_t	PH_MEMLOC_REM wStatus = 0;
	uint8_t		PH_MEMLOC_REM aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH + 2 /* Non-volatile programming bitmask */];
	uint8_t		PH_MEMLOC_REM bCmdLen = 0;
	uint8_t *	PH_MEMLOC_REM pResponse = NULL;
	uint16_t	PH_MEMLOC_REM wRespLen = 0;
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_ActivateOfflineKey(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bOption, uint8_t bKeyNo, uint8_t bKeyV, uint8_t * pDivInput,
	uint8_t bDivInputLen)
{
	...
	/* Verify the parameters. */
	PH_ASSERT_NULL_DATA_PARAM(pDataParams, PH_COMP_HAL);
	if(bDivInputLen) PH_ASSERT_NULL_PARAM(pDivInput, PH_COMP_HAL);
	...
	aCmdBuf[bCmdLen++] = PHHAL_HW_SAMAV3_ISO7816_DEFAULT_P1_BYTE;
	aCmdBuf[bCmdLen++] = PHHAL_HW_SAMAV3_ISO7816_DEFAULT_P2_BYTE;
	aCmdBuf[bCmdLen++] = ((2 /* Key No and Key Version */ + bDivInputLen) & 0xFF);
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_KillAuthentication(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bOption)
{
	...
	if (!bOption)
	{
		...
		memset ( pDataParams->bPendingCmdIv, 0x00, sizeof ( pDataParams->bPendingCmdIv ) );  /* PRQA S 3200 */
		memset ( pDataParams->bPendingCmdMac, 0x00, sizeof ( pDataParams->bPendingCmdMac ) );  /* PRQA S 3200 */
		memset ( pDataParams->bPendingEncCmdData, 0x00, sizeof ( pDataParams->bPendingEncCmdData ) );  /* PRQA S 3200 */
		memset ( pDataParams->bPendingMacCmdData, 0x00, sizeof ( pDataParams->bPendingMacCmdData ) );  /* PRQA S 3200 */
		memset ( pDataParams->bPendingMacRespData, 0x00, sizeof ( pDataParams->bPendingMacRespData ) );  /* PRQA S 3200 */
		memset ( pDataParams->bPendingRespData, 0x00, sizeof ( pDataParams->bPendingRespData ) );  /* PRQA S 3200 */
		memset ( pDataParams->bPendingRespIv, 0x00, sizeof ( pDataParams->bPendingRespIv ) );  /* PRQA S 3200 */
		memset ( pDataParams->bPendingRespMac, 0x00, sizeof ( pDataParams->bPendingRespMac ) );  /* PRQA S 3200 */
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_ChangeKeyEntryOffline(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bKeyNo, uint8_t bProMas, uint16_t wChangeCtr, uint8_t * pOfflineCrypto,
	uint8_t bOfflineCryptoLen, uint8_t bEnableOfflineAck, uint8_t * pOfflineAck)
{
	...
	if(bEnableOfflineAck) PH_ASSERT_NULL_PARAM(pOfflineAck, PH_COMP_HAL);
	...
	if(pDataParams->bKeyNo == bKeyNo)
	{
		...
		PH_CHECK_SUCCESS_FCT(wStatus, phhalHw_SamAV3_Cmd_SAM_KillAuthentication(pDataParams, PHHAL_HW_SAMAV3_CMD_SAM_KILL_AUTHENTICATION_FULL));
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_ChangeKUCEntryOffline(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bKucNo, uint8_t bProMas, uint16_t wChangeCtr, uint8_t * pOfflineCrypto,
	uint8_t bOfflineCryptoLen, uint8_t bEnableOfflineAck, uint8_t * pOfflineAck)
{
	phStatus_t	PH_MEMLOC_REM wStatus = 0;
	uint8_t		PH_MEMLOC_REM aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH + 2 /* ChangeCtr */];
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_DumpSecretKey(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bDumpMode, uint8_t bKeyNo, uint8_t bKeyVer, uint8_t * pDivInput,
	uint8_t bDivInputLen, uint8_t * pSecretKey, uint8_t * pSecretKeyLen)
{
	phStatus_t	PH_MEMLOC_REM wStatus = 0;
	uint8_t		PH_MEMLOC_REM aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH + 2 /* Key number and version */];
	uint8_t	*	PH_MEMLOC_REM pResponse = NULL;
	uint16_t	PH_MEMLOC_REM wRespLen = 0;
	...
	aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_P1_POS]  = bDumpMode;
	aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_P2_POS]  = PHHAL_HW_SAMAV3_ISO7816_DEFAULT_P2_BYTE;
	aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_LC_POS]  = ((2U /* Key number and version */ + bDivInputLen) & 0xFF);
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_DisableKeyEntry(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bKeyNo, uint8_t * pOfflineCrypto, uint8_t bCryptoLen)
{
	...
	/* Parameter validation. */
	PH_ASSERT_NULL_DATA_PARAM(pDataParams, PH_COMP_HAL);
	if(bCryptoLen) PH_ASSERT_NULL_PARAM(pOfflineCrypto, PH_COMP_HAL);
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_DisableKeyEntryOffline(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bKeyNo, uint16_t wChangeCtr, uint8_t * pOfflineCrypto,
	uint8_t bOfflineCryptoLen, uint8_t bEnableOfflineAck, uint8_t * pOfflineAck)
{
	phStatus_t	PH_MEMLOC_REM wStatus = 0;
	uint8_t		PH_MEMLOC_REM aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH + 2 /* ChangeCtr */];
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_EncipherKeyEntry(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bPersoKeyNo, uint8_t bKeyNo, uint8_t bOption, uint16_t wPersoCtr,
	uint8_t * pDivInput, uint8_t bDivInputLen, uint8_t * pOfflineCryptogram, uint8_t * pOfflineCryptogramLen)
{
	phStatus_t	PH_MEMLOC_REM wStatus = 0;
	uint8_t		PH_MEMLOC_REM aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH + 4 /* Logical Channel, KeyNo, Perso Counter*/];
	uint8_t		PH_MEMLOC_REM bCmdLen = 0;
	uint8_t *	PH_MEMLOC_REM pResponse = NULL;
	uint16_t	PH_MEMLOC_REM wRespLen = 0;
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_DeriveKey(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bSrcKeyNo, uint8_t bSrcKeyVer, uint8_t bDstKeyNo, uint8_t * pDeriveIn,
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_GenerateMFCLicMAC(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bOption, uint8_t bInputLen, uint8_t * pInput, uint8_t bKeyCount,
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_ApplySM(phhalHw_SamAV3_DataParams_t * pDataParams, uint16_t wOption, uint8_t bCommMode, uint8_t bOffset, uint8_t bCmdCtrIncr,
	uint8_t * pTxData, uint8_t bTxDataLen, uint8_t ** ppRxData, uint16_t * pRxDataLen)
{
	...
	/* Parameter validation. */
	PH_ASSERT_NULL_DATA_PARAM(pDataParams, PH_COMP_HAL);
	if(bTxDataLen) PH_ASSERT_NULL_PARAM(pTxData, PH_COMP_HAL);
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_RemoveSM(phhalHw_SamAV3_DataParams_t * pDataParams, uint16_t wOption, uint8_t bCommMode, uint8_t * pTxData, uint8_t bTxDataLen,
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_GenerateMAC(phhalHw_SamAV3_DataParams_t * pDataParams, uint16_t wOption, uint8_t bNum, uint8_t * pTxData, uint8_t bTxDataLen,
	uint8_t ** ppRxData, uint16_t * pRxDataLen)
{
	phStatus_t	PH_MEMLOC_REM wStatus = 0;
	uint8_t		PH_MEMLOC_REM aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH];
	...
	PH_ASSERT_NULL_DATA_PARAM(pDataParams, PH_COMP_HAL);
	if(bTxDataLen) PH_ASSERT_NULL_PARAM(pTxData, PH_COMP_HAL);
	if (!(wOption & PH_EXCHANGE_BUFFERED_BIT)) PH_ASSERT_NULL_PARAM(pRxDataLen, PH_COMP_HAL);
	...
	if (!(wOption & PH_EXCHANGE_BUFFERED_BIT))
	{
		...
		if (pDataParams->pTxBuffer[PHHAL_HW_SAMAV3_ISO7816_P1_POS] != PHHAL_HW_SAMAV3_ISO7816_CHAINED_FRAME)
		{
			wStatus = phhalHw_SamAV3_Cmd_7816Exchange(
				pDataParams,
				PH_EXCHANGE_BUFFER_CONT,
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_DecipherData(phhalHw_SamAV3_DataParams_t * pDataParams, uint16_t wOption, uint8_t * pEncData, uint8_t bEncDataLen, uint8_t * pLength,
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_EncipherData(phhalHw_SamAV3_DataParams_t * pDataParams, uint16_t wOption, uint8_t * pPlainData, uint8_t bPlainDataLen, uint8_t bOffset,
	uint8_t ** ppEncData, uint16_t * pEncDataLen)
{
	...
	/* Parameter validation. */
	PH_ASSERT_NULL_DATA_PARAM(pDataParams, PH_COMP_HAL);
	if(bPlainDataLen) PH_ASSERT_NULL_PARAM(pPlainData, PH_COMP_HAL);
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_DecipherOfflineData(phhalHw_SamAV3_DataParams_t * pDataParams, uint16_t wOption, uint8_t * pEncData, uint8_t bEncDataLen,
	uint8_t ** ppPlainData, uint16_t * pPlainDataLen)
{
	phStatus_t	PH_MEMLOC_REM wStatus = 0;
	uint8_t		PH_MEMLOC_REM aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH];
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_EncipherOfflineData(phhalHw_SamAV3_DataParams_t * pDataParams, uint16_t wOption, uint8_t * pPlainData, uint8_t bPlainDataLen,
    uint8_t ** ppEncData, uint16_t * pEncDataLen)
{
	phStatus_t	PH_MEMLOC_REM wStatus = 0;
	uint8_t		PH_MEMLOC_REM aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH];
...
phStatus_t phhalHw_SamAV3_Cmd_PKI_GenerateKeyPair(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bOption, uint8_t bPKI_KeyNo, uint16_t wPKI_Set, uint8_t bPKI_KeyNoCEK,
	uint8_t bPKI_KeyVCEK, uint8_t bPKI_RefNoKUC, uint8_t bPKI_KeyNoAEK, uint8_t bPKI_KeyVAEK, uint16_t wPKI_NLen, uint16_t wPKI_eLen, uint8_t * pPKI_e)
{
	phStatus_t	PH_MEMLOC_REM wStatus = 0;
	...
	else
	{
		...
		if(bOption & PHHAL_HW_SAMAV3_CMD_PKI_GENERATE_KEY_PAIR_HOST_E)
		{
			aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_LC_POS] = ((aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_LC_POS] + wPKI_eLen) & 0xFF);
		}
	}
...
phStatus_t phhalHw_SamAV3_Cmd_PKI_ImportKey(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bOption, uint8_t bPKI_KeyNo, uint16_t wPKI_Set, uint8_t bPKI_KeyNoCEK,
	uint8_t bPKI_KeyVCEK, uint8_t bPKI_RefNoKUC, uint8_t bPKI_KeyNoAEK, uint8_t bPKI_KeyVAEK, uint16_t wPKI_NLen, uint16_t wPKI_eLen, uint16_t wPKI_PLen, uint16_t wPKI_QLen,
	uint8_t * pPKI_N, uint8_t * pPKI_e, uint8_t * pPKI_p, uint8_t * pPKI_q, uint8_t * pPKI_dP, uint8_t * pPKI_dQ, uint8_t * pPKI_ipq)
{
	phStatus_t	PH_MEMLOC_REM wStatus = 0;
	...
	/* Parameter validation. */
	if ((bOption & PHHAL_HW_SAMAV3_CMD_PKI_UPDATE_KEY_SETTINGS_ONLY) != PHHAL_HW_SAMAV3_CMD_PKI_UPDATE_KEY_SETTINGS_ONLY)
	{
	...
	/* Update LC byte if Access Entry Key information is available. */
	if((bOption & PHHAL_HW_SAMAV3_CMD_PKI_IMPORT_KEY_AEK_INCLUDE) == PHHAL_HW_SAMAV3_CMD_PKI_IMPORT_KEY_AEK_INCLUDE)
		aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_LC_POS] += 2 /* PKI_KeyNoAEK and PKI_KeyVAEK. */;
	...
	wRemainingMsgLen += ((wPKI_NLen + wPKI_eLen) & 0xFFFF);
	...
	if (bIsPrivateKeyIncluded)
	{
		...
		wRemainingMsgLen += (((wPKI_PLen * 2) + (wPKI_QLen * 3)) & 0xFFFF);
	}

	/* Update Remaining message length valiable with the key entry length componenets (wCmdLen - I7816 Header). */
	wRemainingMsgLen = ((wRemainingMsgLen + (wCmdLen - PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH)) & 0xFF);
	...
	do
	{
		...
		if (wPayLoadSize > 0)
		{
			...
			{
				wPayLoadSize =(((PHHAL_HW_SAMAV3_ISO7816_MAX_LC_MULTIPLE_AESBLOCK - 16) - wCmdLen) & 0xFFFF);
...
phStatus_t phhalHw_SamAV3_Cmd_PKI_ExportPrivateKey(phhalHw_SamAV3_DataParams_t * pDataParams, uint16_t wOption, uint8_t bPKI_KeyNo, uint8_t ** ppKeyData, uint16_t *pKeyDataLen)
{
	...
	aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_P2_POS] = (uint8_t) (wOption & PHHAL_HW_SAMAV3_CMD_PKI_EXPORT_PRIVATE_KEY_AEK_INCLUDE);
	aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_LE_NO_LC_POS] = PHHAL_HW_SAMAV3_ISO7816_DEFAULT_LE_BYTE;
...
phStatus_t phhalHw_SamAV3_Cmd_PKI_ExportPublicKey(phhalHw_SamAV3_DataParams_t * pDataParams, uint16_t wOption, uint8_t bPKI_KeyNo, uint8_t ** ppKeyData, uint16_t *pKeyDataLen)
{
	...
	aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_P1_POS] = bPKI_KeyNo;
	aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_P2_POS] = (uint8_t) (wOption & PHHAL_HW_SAMAV3_CMD_PKI_EXPORT_PUBLIC_KEY_AEK_INCLUDE);
	aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_LE_NO_LC_POS] = PHHAL_HW_SAMAV3_ISO7816_DEFAULT_LE_BYTE;
...
phStatus_t phhalHw_SamAV3_Cmd_PKI_UpdateKeyEntries(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bOption, uint8_t bNoOfKeyEntries, uint8_t bHashingAlg,
	uint8_t bPKI_KeyNo_Enc, uint8_t bPKI_KeyNo_Sign, uint8_t bPKI_KeyNo_Ack, uint8_t * pKeyFrame, uint16_t wKeyFrameLen, uint8_t ** ppUpdateACK, uint16_t * pUpdateACKLen)
{
	phStatus_t	PH_MEMLOC_REM wStatus = 0;
	uint8_t		PH_MEMLOC_REM aCmdBuf[PHHAL_HW_SAMAV3_CMD_PKI_IMPORT_KEY_CMD_SIZE];
	uint8_t		PH_MEMLOC_REM bCmdLen = 0;
	...
	else
	{
		...
		do
		{
			...
			if ((uint16_t) (wKeyFrameLen - wBufPos) > (uint16_t) (PHHAL_HW_SAMAV3_ISO7816_MAX_LC_MULTIPLE_AESBLOCK - bPayloadLen))
			{
				PH_CHECK_SUCCESS_FCT(wStatus, phhalHw_SamAV3_Utils_UpdateP2(pDataParams, PHHAL_HW_SAMAV3_ISO7816_CHAINED_FRAME));
...
phStatus_t phhalHw_SamAV3_Cmd_PKI_EncipherKeyEntries(phhalHw_SamAV3_DataParams_t * pDataParams, uint16_t wOption, uint8_t bNoOfKeyEntries, uint8_t bHashingAlg, uint8_t bPKI_KeyNo_Enc,
	uint8_t bPKI_KeyNo_Sign, uint8_t bPKI_KeyNo_Dec, uint8_t bPKI_KeyNo_Verif, uint16_t wPerso_Ctr, uint8_t * pKeyEntries, uint8_t bKeyEntriesLen, uint8_t * pDivInput, uint8_t bDivInputLen,
	uint8_t ** ppEncKeyFrame_Sign, uint16_t * pEncKeyFrame_Sign_Len)
{
	...
	{
		/* Update P1 information byte. */
		aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_P1_POS] = (uint8_t) (wOption & PHHAL_HW_SAMAV3_CMD_PKI_DIVERSIFICATION_ON);
		aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_P1_POS] |= (((bNoOfKeyEntries << 2) | bHashingAlg) & 0xFF);
...
phStatus_t phhalHw_SamAV3_Cmd_PKI_GenerateHash(phhalHw_SamAV3_DataParams_t * pDataParams, uint16_t wOption, uint8_t bHashingAlg, uint32_t dwMLen, uint8_t * pMessage, uint16_t wMsgLen,
	uint8_t ** ppHash, uint16_t * pHashLen)
{
	...
	if (!(wOption & PH_EXCHANGE_LEAVE_BUFFER_BIT))
	{
		...
		if (wOption & PHHAL_HW_SAMAV3_CMD_PKI_GENERATE_HASH_FIRST_FRAME)
		{
			...
			aCmdBuf[bCmdLen++] = ((dwMLen >> 24) & 0xFF);
			aCmdBuf[bCmdLen++] = ((dwMLen >> 16) & 0xFF);
			aCmdBuf[bCmdLen++] = ((dwMLen >> 8) & 0xFF);
			aCmdBuf[bCmdLen++] = ((dwMLen >> 0) & 0xFF);
		}
...
{
	phStatus_t  PH_MEMLOC_REM wStatus = 0;
	uint8_t     PH_MEMLOC_REM aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH + 1 /* bPKI_KeyNo_Sign */];
	uint8_t *   PH_MEMLOC_REM pResponse = NULL;
	uint16_t    PH_MEMLOC_REM wRespLen = 0;
	...
	aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_P1_POS] = bHashingAlg;
	aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_P2_POS] = PHHAL_HW_SAMAV3_ISO7816_DEFAULT_P2_BYTE;
	aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_LC_POS] = ((1U /* bPKI_KeyNo_Sign */ + bHashLen) & 0xFF);
...
phStatus_t phhalHw_SamAV3_Cmd_PKI_VerifySignature(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bPKI_KeyNo_Verif, uint8_t bHashingAlg, uint8_t * pHash, uint8_t bHashLen,
	uint8_t * pSignature, uint16_t wSignatureLen)
{
	...
	aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_P1_POS] = bHashingAlg;
	aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_P2_POS] = PHHAL_HW_SAMAV3_ISO7816_DEFAULT_P2_BYTE;
	aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_LC_POS] = ((1 /* bPKI_KeyNo_Verif */ + bHashLen + wSignatureLen) & 0xFF);
	...
	if ((1 /* bPKI_KeyNo_Verif */ + bHashLen + wSignatureLen) > PHHAL_HW_SAMAV3_ISO7816_MAX_LC_MULTIPLE_AESBLOCK)
	{
		aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_P2_POS] = PHHAL_HW_SAMAV3_ISO7816_CHAINED_FRAME;
	...
	else
	{
        ...
        if((PHHAL_HW_SAMAV3_ISO7816_MAX_LC_MULTIPLE_AESBLOCK - 1U - bHashLen) <= 0)
        {
            bBufPos = 0;
        }
        else
        {
            bBufPos = ((PHHAL_HW_SAMAV3_ISO7816_MAX_LC_MULTIPLE_AESBLOCK - 1U - bHashLen) & 0xFF);
        }
		...
		aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_P1_POS] = PHHAL_HW_SAMAV3_ISO7816_DEFAULT_P1_BYTE;
		aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_P2_POS] = PHHAL_HW_SAMAV3_ISO7816_DEFAULT_P2_BYTE;
		aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_LC_POS] = (uint8_t)((wSignatureLen - bBufPos) & 0xFF);
...
phStatus_t phhalHw_SamAV3_Cmd_PKI_EncipherData(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bHashingAlg, uint8_t bPKI_KeyNo_Enc, uint8_t * pPlainData, uint16_t wPlainDataLen,
	uint8_t ** ppEncData, uint16_t * pEncDataLen)
{
	phStatus_t	PH_MEMLOC_REM wStatus = 0;
	...
	aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_P1_POS] = bHashingAlg;
	aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_P2_POS] = PHHAL_HW_SAMAV3_ISO7816_DEFAULT_P2_BYTE;
	aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_LC_POS] = ((1U /* bPKI_KeyNo_Enc */ + wPlainDataLen) & 0xFF);
...
phStatus_t phhalHw_SamAV3_Cmd_PKI_DecipherData(phhalHw_SamAV3_DataParams_t * pDataParams, uint16_t wOption, uint8_t bHashingAlg, uint8_t bPKI_KeyNo_Dec, uint8_t * pEncData,
	uint16_t wEncDataLen, uint8_t ** ppPlainData, uint16_t * pPlainDataLen)
{
	...
	if((wOption & PH_EXCHANGE_CUSTOM_BITS_MASK) == PHHAL_HW_SAMAV3_CMD_PKI_DECIPHER_DATA_FIRST_FRAME)
	{
		aCmdBuf[bCmdLen++] = bPKI_KeyNo_Dec;
	}
...
phStatus_t phhalHw_SamAV3_Cmd_PKI_ImportEccKey(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bOption, uint8_t bECC_KeyNo, uint16_t wECC_Set, uint8_t bECC_KeyNoCEK,
	uint8_t bECC_KeyVCEK, uint8_t bECC_RefNoKUC, uint8_t bECC_KeyNoAEK, uint8_t bECC_KeyVAEK, uint16_t wECC_Len, uint8_t * pECC_xy, uint8_t bECC_xyLen)
{
	phStatus_t	PH_MEMLOC_REM wStatus = 0;
	...
	{
		/* Update LC */
		aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_LC_POS] = ((aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_LC_POS] + (((uint8_t)2U /* ECC_Len */ + bECC_xyLen) & 0xFF)) & 0xFF);
...
phStatus_t phhalHw_SamAV3_Cmd_PKI_ImportEccCurve(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bOption, uint8_t bECC_CurveNo, uint8_t bECC_KeyNoCCK, uint8_t bECC_KeyVCCK,
	uint8_t bECC_N, uint8_t bECC_M, uint8_t * pECC_Prime, uint8_t * pECC_ParamA, uint8_t * pECC_ParamB, uint8_t * pECC_Px, uint8_t * pECC_Py, uint8_t * pECC_Order)
{
	phStatus_t	PH_MEMLOC_REM wStatus = 0;
	...
	{
		/* Update LC */
        aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_LC_POS] += ((2 /* ECC_N, ECC_M */ +
			(5 /* ECC_Prime, ECC_ParamA, ECC_ParamB, ECC_Px, ECC_Py, ECC_Order */ * bECC_N) + bECC_M ) & 0xFF);
...
phStatus_t phhalHw_SamAV3_Cmd_PKI_ExportEccPublicKey(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bECC_KeyNo, uint16_t * pECC_Set, uint8_t * pECC_KeyNoCEK,
	uint8_t * pECC_KeyVCEK, uint8_t * pECC_RefNoKUC, uint8_t * pECC_KeyNoAEK, uint8_t * pECC_KeyVAEK, uint16_t * pECC_Len, uint8_t ** ppECC_xy, uint8_t * pECC_xyLen)
{
	phStatus_t	PH_MEMLOC_REM wStatus = 0;
	uint8_t		PH_MEMLOC_REM aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH];
	uint8_t *	PH_MEMLOC_REM pResponse = NULL;
	...
	*pECC_Set = ((((uint16_t)pResponse[0] << 8) | pResponse[1]) & 0xFFFF);
	...
	*pECC_Len = ((((uint16_t)pResponse[7] << 8) | pResponse[8]) & 0xFFFF);

	*ppECC_xy = &pResponse[9];
	*pECC_xyLen = ((wRespLen - 9) & 0xFF);

	return PH_ADD_COMPCODE(PH_ERR_SUCCESS, PH_COMP_HAL);
}

phStatus_t phhalHw_SamAV3_Cmd_PKI_VerifyEccSignature(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bECC_KeyNo, uint8_t bECC_CurveNo, uint8_t bLen, uint8_t * pMessage,
	uint8_t * pSignature, uint16_t wSignatureLen)
{
	phStatus_t	PH_MEMLOC_REM wStatus = 0;
	...
	aCmdBuf[bCmdLen++] = PHHAL_HW_SAMAV3_ISO7816_DEFAULT_P1_BYTE;
	aCmdBuf[bCmdLen++] = PHHAL_HW_SAMAV3_ISO7816_DEFAULT_P2_BYTE;
	aCmdBuf[bCmdLen++] = ((PHHAL_HW_SAMAV3_CMD_PKI_VERIFY_ECC_SIGNATURE_LC_MIN + bLen + wSignatureLen) & 0xFF);
...
phStatus_t phhalHw_SamAV3_Cmd_PKI_ImportCaPk(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bOption, uint8_t * pCaPkData, uint8_t bCaPkDataLen)
{
	...
	/* Validate the parameters. */
	PH_ASSERT_NULL_DATA_PARAM(pDataParams, PH_COMP_HAL);
	if(bCaPkDataLen) PH_ASSERT_NULL_PARAM(pCaPkData, PH_COMP_HAL);
	...
	do
	{
		...
		{
			PH_CHECK_SUCCESS_FCT(wStatus, phhalHw_SamAV3_Utils_UpdateP1(pDataParams, PHHAL_HW_SAMAV3_ISO7816_CHAINED_FRAME));
		...
		{
			PH_CHECK_SUCCESS_FCT(wStatus, phhalHw_SamAV3_Utils_UpdateP1(pDataParams, PHHAL_HW_SAMAV3_ISO7816_LAST_FRAME));
...
phStatus_t phhalHw_SamAV3_Cmd_PKI_ImportCaPkOffline(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bReceiveOfflineAck, uint8_t bOption, uint8_t * pOfflineCryptogram,
	uint8_t bOfflineCryptogramLen, uint8_t ** ppOfflineAck, uint16_t * pOfflineAckLen)
{
	...
	/* Validate the parameters. */
	PH_ASSERT_NULL_DATA_PARAM(pDataParams, PH_COMP_HAL);
	if(bOfflineCryptogramLen) PH_ASSERT_NULL_PARAM(pOfflineCryptogram, PH_COMP_HAL);
	if(bReceiveOfflineAck) PH_ASSERT_NULL_PARAM(pOfflineAckLen, PH_COMP_HAL);
	...
	do
	{
		...
		if ((wStatus & PH_ERR_MASK) == PHHAL_HW_SAMAV3_ERR_OK_CHAINING_ACTIVE)
		{
			...
			bOfflineCryptoLen = (uint8_t) (bOfflineCryptogramLen - 250 /* Max Offline Crpytogram data that can fit in one frame. */);
		}
...
phStatus_t phhalHw_SamAV3_Cmd_PKI_RemoveCaPkOffline(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bReceiveOfflineAck, uint8_t * pOfflineCryptogram, uint8_t bOfflineCryptogramLen,
...
phStatus_t phhalHw_SamAV3_Cmd_PKI_LoadIssuerPk(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bLFI, uint16_t wOption, uint8_t * pData, uint8_t bDataLen, uint8_t * pIssureID,
	uint8_t * pExpDate, uint8_t * pSerialNo)
{
		...
		((wOption & PH_EXCHANGE_BUFFER_MASK) == PH_EXCHANGE_BUFFER_FIRST))
	{
		...
		aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_CLA_POS] = PHHAL_HW_SAMAV3_ISO7816_CLA_BYTE;
		aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_INS_POS] = PHHAL_HW_SAMAV3_CMD_PKI_LOAD_ISSUER_PK_INS;
...
phStatus_t phhalHw_SamAV3_Cmd_PKI_LoadIccPk(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bLFI, uint16_t wOption, uint8_t * pData, uint8_t bDataLen, uint8_t * pPAN, uint8_t * pExpDate,
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_SelectVC(phhalHw_SamAV3_DataParams_t *pDataParams, uint8_t bOption, uint8_t bEncKeyNo, uint8_t bEncKeyVer, uint8_t bMacKeyNo,
	uint8_t bMacKeyVer, uint8_t * pData, uint8_t bDataLen, uint8_t * pDivInput, uint8_t  bDivInputLen, uint8_t ** ppResponse, uint16_t * pRespLen)
{
	phStatus_t	PH_MEMLOC_REM wStatus = 0;
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_ProximityCheck_Part1(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t  bOption, uint8_t  bKeyNo, uint8_t  bKeyVer,
	uint8_t * pPPCData, uint8_t  bPPCDataLen, uint8_t * pPCData, uint8_t  bPCDataLen, uint8_t * pDivInput, uint8_t bDivInputLen, uint8_t ** ppMac, uint16_t * pMacLen )
{
	phStatus_t	PH_MEMLOC_REM wStatus = 0;
    uint8_t		PH_MEMLOC_REM aCmdBuff[PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH + 3 /* Key No, Key Ver and PPC Data Length */];
	uint8_t		PH_MEMLOC_REM bCmdLen = 0;
	...
	if(bOption) PH_ASSERT_NULL_PARAM(pDivInput, PH_COMP_HAL);
...
phStatus_t phhalHw_SamAV3_Cmd_VCA_Select(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bOption, uint8_t bEncKeyNo, uint8_t bEncKeyVer, uint8_t bMacKeyNo,
...
{
	phStatus_t	PH_MEMLOC_REM wStatus = 0;
    uint8_t		PH_MEMLOC_REM aCmdBuff[PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH + 5 /* Key Information, IID len */];
	uint8_t		PH_MEMLOC_REM bCmdLen = 0;
	uint8_t *	PH_MEMLOC_REM pResponse = NULL;
	uint16_t	PH_MEMLOC_REM wRespLen = 0;
...
phStatus_t phhalHw_SamAV3_Cmd_VCA_ProximityCheck(phhalHw_SamAV3_DataParams_t *pDataParams, uint8_t bOption, uint8_t bKeyNo, uint8_t bKeyVer, uint8_t bNumOfRand,
	uint8_t * pDivInput, uint8_t bDivInputLen, uint8_t ** ppResponse, uint16_t * pRespLen)
{
	...
	/* Validate the parameters. */
	PH_ASSERT_NULL_DATA_PARAM(pDataParams, PH_COMP_HAL);
	if(bOption & PHHAL_HW_SAMAV3_CMD_PROXIMITY_CHECK_DIV_ON) PH_ASSERT_NULL_PARAM(pDivInput, PH_COMP_HAL);
	PH_ASSERT_NULL_PARAM(pRespLen, PH_COMP_HAL);
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_AuthenticatePICC_Part1(phhalHw_SamAV3_DataParams_t * pDataParams,	uint8_t bOption, uint8_t bKeyNo, uint8_t bKeyVer,
	uint8_t bAuthMode, uint8_t * pDivInput, uint8_t bDivInputLen, uint8_t * pCardResponse, uint8_t bCardRespLen, uint8_t ** ppSamResponse,
	uint16_t * pSamRespLen)
{
	phStatus_t	PH_MEMLOC_REM wStatus = 0;
	uint8_t		PH_MEMLOC_REM aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH + 3 /* Key No, Key Ver and AuthMode */ ];
	uint8_t		PH_MEMLOC_REM bCmdLen = 0;

	/* Validate the parameters. */
	PH_ASSERT_NULL_DATA_PARAM(pDataParams, PH_COMP_HAL);
	if(bOption & PHHAL_HW_CMD_SAMAV3_KEY_DIVERSIFICATION_ON) PH_ASSERT_NULL_PARAM(pDivInput, PH_COMP_HAL);
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_AuthenticatePICC_Part2(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bPiccErrorCode, uint8_t * pCardResponse,
	uint8_t bCardRespLen, uint8_t * pPDcap2, uint8_t * pPCDcap2, uint8_t * pStatusCode)
{
	 ...
	 */
	if((bPiccErrorCode != 0x00 /* Picc status code */) || (bCardRespLen == 0x20 /* PICC response Length */))
	{
		wStatus = phhalHw_SamAV3_Cmd_7816Exchange(
			pDataParams,
			PH_EXCHANGE_BUFFER_LAST,
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_IsoAuthenticatePICC_Part1(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bOption, uint8_t bKeyNo, uint8_t bKeyVer,
	uint8_t * pDivInput, uint8_t bDivInputLen, uint8_t * pCardResponse, uint8_t bCardRespLen, uint8_t ** ppSamResponse, uint16_t * pSamRespLen)
{
	phStatus_t	PH_MEMLOC_REM wStatus = 0;
	uint8_t		PH_MEMLOC_REM aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH + 2 /* Key No and Key Ver */ ];
	uint8_t		PH_MEMLOC_REM bCmdLen = 0;
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_ChangeKeyPICC(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bCryptoMethod, uint8_t bConfig, uint8_t bKeySetNo,
	uint8_t bDFKeyNo, uint8_t bCurrKeyNo, uint8_t bCurrKeyVer, uint8_t bNewKeyNo, uint8_t bNewKeyVer, uint8_t * pDivInput, uint8_t bDivInputLen,
	uint8_t ** ppSamResponse, uint16_t * pSamRespLen)
{
	phStatus_t	PH_MEMLOC_REM wStatus = 0;
	uint8_t		PH_MEMLOC_REM aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH + 6 /* Key information, PCDcap len. */];
	uint8_t		PH_MEMLOC_REM bCmdLen = 0;
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_CreateTMFilePICC(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bOption, uint8_t bKeyNo, uint8_t bKeyVer, uint8_t bFileNo,
	uint8_t bFileOption, uint8_t * pAccessRights, uint8_t bTMKeyOptions, uint8_t * pDivInput, uint8_t bDivInputLen, uint8_t ** ppSamResponse, uint16_t * pSamRespLen)
{
	phStatus_t	PH_MEMLOC_REM wStatus = 0;
	uint8_t		PH_MEMLOC_REM aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH + 7 /* TM File information */];
	uint8_t		PH_MEMLOC_REM bCmdLen= 0;
...
phStatus_t phhalHw_SamAV3_Cmd_DESFire_AuthenticatePICC(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bOption, uint8_t bISOMode, uint8_t bDFKeyNo,
	uint8_t bKeyNo, uint8_t bKeyVer, uint8_t bPCDcap2InLen, uint8_t * pPCDcap2In, uint8_t * pDivInput, uint8_t  bDivInputLen, uint8_t * pPDcap2,
	uint8_t * pPCDcap2, uint8_t * pPiccReturnCode)
{
	phStatus_t	PH_MEMLOC_REM wStatus = 0;
	uint8_t		PH_MEMLOC_REM aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH + 4 /* Key information, PCDcap len. */];
	uint8_t		PH_MEMLOC_REM bCmdLen = 0;
	uint8_t *	PH_MEMLOC_REM pResponse = NULL;
	uint16_t	PH_MEMLOC_REM wRespLen = 0;
...
phStatus_t phhalHw_SamAV3_Cmd_DESFire_ChangeKeyPICC(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bKeyCompMeth, uint8_t bConfig, uint8_t bKeySetNo,
	uint8_t bDFKeyNo, uint8_t bCurrKeyNo, uint8_t bCurrKeyVer, uint8_t bNewKeyNo, uint8_t bNewKeyVer, uint8_t * pDivInput, uint8_t  bDivInputLen,
...
phStatus_t phhalHw_SamAV3_Cmd_DESFire_WriteX(phhalHw_SamAV3_DataParams_t * pDataParams, uint16_t wOption, uint8_t bCrypto, uint8_t * pData, uint8_t bDataLen,
...
phStatus_t phhalHw_SamAV3_Cmd_DESFire_ReadX(phhalHw_SamAV3_DataParams_t * pDataParams, uint16_t wOption, uint8_t bCrypto, uint8_t * pAppData, uint8_t bAppDataLen,
	uint8_t ** ppResponse, uint16_t * pRespLen, uint8_t * pPiccReturnCode, uint8_t * pErrLen)
{
	...
	if((bCrypto == PHHAL_HW_CMD_SAMAV3_CRYPTO_CONFIG_COMM_MODE_FULL) && !(wOption & PH_EXCHANGE_RXCHAINING))
	{
	...
	PH_CHECK_SUCCESS_FCT(wStatus, phhalHw_SamAV3_Cmd_7816Exchange(
		...
		(uint16_t)((bAppDataLen - bAppDataOffset) & 0xFF),
		NULL,
		NULL));
...
phStatus_t phhalHw_SamAV3_Cmd_DESFire_CreateTMFilePICC(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bOption, uint8_t bISOMode, uint8_t bKeyNo, uint8_t bKeyVer,
	uint8_t bFileNo, uint8_t bFileOption, uint8_t * pAccessRights, uint8_t bTMKeyOptions, uint8_t * pDivInput, uint8_t bDivInputLen, uint8_t * pPiccReturnCode)
{
	phStatus_t	PH_MEMLOC_REM wStatus = 0;
	uint8_t		PH_MEMLOC_REM aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH + 7 /* TM File information */];
	uint8_t		PH_MEMLOC_REM bCmdLen= 0;
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_AuthenticateMFP_Part1(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bOption, uint8_t bKeyNo,
	...
	uint16_t * pPCDChalRespLen)
{
	...
	/* Verify the parameter. */
	PH_ASSERT_NULL_DATA_PARAM(pDataParams, PH_COMP_HAL);
	if(bPDChalLen) PH_ASSERT_NULL_PARAM(pPDChal, PH_COMP_HAL);
	if(bDivInputLen) PH_ASSERT_NULL_PARAM(pDivInput, PH_COMP_HAL);
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_AuthenticateMFP_Part2(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bPiccErrCode, uint8_t * pPDResp,
		uint8_t bPDRespLen, uint8_t ** ppPDCap2, uint8_t ** ppPCDCap2, uint8_t * pPiccReturnCode)
{
	...
	/* Verify the parameter. */
	PH_ASSERT_NULL_DATA_PARAM(pDataParams, PH_COMP_HAL);
	if(bPDRespLen) PH_ASSERT_NULL_PARAM(pPDResp, PH_COMP_HAL);
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_AuthSectorSwitchMFP_Part1(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bOption, uint8_t * pPDChal,
	uint8_t bPDChalLen, uint16_t wSSKeyBNr, uint8_t bSSKeyNo, uint8_t bSSKeyVer, uint8_t bMSKeyNo, uint8_t bMSKeyVer, uint8_t bSectorCount,
	uint8_t *pKeyBlocks, uint8_t bKeyBlocksLen, uint8_t * pDivInput, uint8_t bDivInputLen, uint8_t ** ppPCDChalResp, uint16_t * pPCDChalRespLen)
{
	phStatus_t	PH_MEMLOC_REM wStatus = 0;
	uint8_t		PH_MEMLOC_REM aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH + 2 /* For payload information. */];
	uint8_t		PH_MEMLOC_REM bCmdLen = 0;

	/* Verify the parameter. */
	PH_ASSERT_NULL_DATA_PARAM(pDataParams, PH_COMP_HAL);
	if(bPDChalLen) PH_ASSERT_NULL_PARAM(pPDChal, PH_COMP_HAL);
	if(bKeyBlocksLen) PH_ASSERT_NULL_PARAM(pKeyBlocks, PH_COMP_HAL);
	if(bDivInputLen) PH_ASSERT_NULL_PARAM(pDivInput, PH_COMP_HAL);
	PH_ASSERT_NULL_PARAM(pPCDChalRespLen, PH_COMP_HAL);
	...
	if((bOption & PHHAL_HW_SAMAV3_MFP_SSAUTH_MASTER_SECTOR_DIV_ON) == PHHAL_HW_SAMAV3_MFP_SSAUTH_MASTER_SECTOR_DIV_ON)
	{
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_AuthSectorSwitchMFP_Part2(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bPiccErrCode, uint8_t * pPDResp,
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_AuthenticatePDC_Part1(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bOption, uint8_t bKeyNo,
	...
	uint8_t bDivInputLen, uint8_t ** ppPCDChalResp, uint16_t * pPCDChalRespLen)
{
	...
	PH_ASSERT_NULL_DATA_PARAM(pDataParams, PH_COMP_HAL);
	if(bPDChalLen) PH_ASSERT_NULL_PARAM(pPDChal, PH_COMP_HAL);
	if(bLen) PH_ASSERT_NULL_PARAM(pUpgradeInfo, PH_COMP_HAL);
	PH_ASSERT_NULL_PARAM(pPCDChalRespLen, PH_COMP_HAL);
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_AuthenticatePDC_Part2(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bPiccErrCode, uint8_t * pPDResp,
	uint8_t bPDRespLen, uint8_t * pPiccReturnCode)
{
	phStatus_t	PH_MEMLOC_REM wStatus = 0;
	uint8_t		PH_MEMLOC_REM aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH];
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_CombinedReadMFP(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bLFI, uint16_t wOption, uint8_t * pData,
	uint8_t bDataLen, uint8_t ** ppOutput, uint16_t * pOutputLen, uint8_t * pPiccReturnCode)
{
	...
	PH_ASSERT_NULL_DATA_PARAM(pDataParams, PH_COMP_HAL);
	if(bDataLen) PH_ASSERT_NULL_PARAM(pData, PH_COMP_HAL);
	if(((wOption & PHHAL_HW_SAMAV3_OPTION_COMBINED_READ_MFP_RESPONSE) == PHHAL_HW_SAMAV3_OPTION_COMBINED_READ_MFP_RESPONSE) ||
		((wOption & PHHAL_HW_SAMAV3_OPTION_COMBINED_READ_MFP_BOTH) == PHHAL_HW_SAMAV3_OPTION_COMBINED_READ_MFP_BOTH))
	{
		if(((wOption & PH_EXCHANGE_BUFFER_MASK) == PH_EXCHANGE_DEFAULT) || ((wOption & PH_EXCHANGE_BUFFER_MASK) == PH_EXCHANGE_BUFFER_LAST))
		{
		...
		((wOption & PH_EXCHANGE_BUFFER_MASK) == PH_EXCHANGE_DEFAULT))
	{
		...
		aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_P1_POS]	= (uint8_t) (wOption & PHHAL_HW_SAMAV3_OPTION_COMBINED_READ_MASK);
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_CombinedWriteMFP(phhalHw_SamAV3_DataParams_t * pDataParams, uint16_t wOption, uint8_t * pData, uint8_t bDataLen,
	uint8_t ** ppOutput, uint16_t * pOutputLen, uint8_t * pPiccReturnCode)
{
	...
	if(((wOption & PH_EXCHANGE_BUFFER_MASK) == PH_EXCHANGE_DEFAULT) || ((wOption & PH_EXCHANGE_BUFFER_MASK) == PH_EXCHANGE_BUFFER_LAST))
	{
		if((wOption & PHHAL_HW_SAMAV3_OPTION_COMBINED_WRITE_MFP_RESPONSE) == PHHAL_HW_SAMAV3_OPTION_COMBINED_WRITE_MFP_RESPONSE)
		{
			PH_ASSERT_NULL_PARAM(pPiccReturnCode, PH_COMP_HAL);
		}
	}

	if(((wOption & PH_EXCHANGE_BUFFER_MASK) == PH_EXCHANGE_DEFAULT) || ((wOption & PH_EXCHANGE_BUFFER_MASK) == PH_EXCHANGE_BUFFER_FIRST))
	{
		/* Reset the command buffer.*/
		memset(aCmdBuf, 0x00, sizeof(aCmdBuf));		/* PRQA S 3200 */
		...
		/* Update P1 information byte.  */
		aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_P1_POS]  |= (uint8_t) (wOption & PHHAL_HW_SAMAV3_OPTION_COMBINED_WRITE_MASK);
		aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_P1_POS]  |= (uint8_t) (wOption & PHHAL_HW_SAMAV3_OPTION_COMBINED_WRITE_PLIAN_RESPONSE_MASK);
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_ChangeKeyMFP(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bOption, uint8_t * pData, uint8_t bDataLen,
	uint8_t ** ppProtectedData, uint16_t * pProtectedDataLen, uint8_t * pPiccReturnCode)
{
	...
	if((bOption & PHHAL_HW_SAMAV3_OPTION_MFP_CHANGE_KEY_RESPONSE) == PHHAL_HW_SAMAV3_OPTION_MFP_CHANGE_KEY_RESPONSE)
	{
		PH_ASSERT_NULL_PARAM(pPiccReturnCode, PH_COMP_HAL);
	}
...
phStatus_t phhalHw_SamAV3_Cmd_MFP_Authenticate(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bOption, uint8_t bKeyNo, uint8_t bKeyVer,
	uint16_t wBlockNo, uint8_t * pPcdCapsIn, uint8_t bPcdCapsInLen, uint8_t * pDivInput, uint8_t bDivInputLen, uint8_t * pPcdCapsOut,
	uint8_t * pPdCaps, uint8_t * pPiccReturnCode)
{
	phStatus_t	PH_MEMLOC_REM wStatus = 0;
	uint8_t		PH_MEMLOC_REM aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH + 5 /* KeyNo, KeyVer, BNR, PcdCapInLen */];
	uint8_t		PH_MEMLOC_REM bCmdLen = 0;
	uint8_t *	PH_MEMLOC_REM pResponse = NULL;
	uint16_t	PH_MEMLOC_REM wRespLen = 0;
	...
	aCmdBuf[bCmdLen++] = bOption;
	aCmdBuf[bCmdLen++] = PHHAL_HW_SAMAV3_ISO7816_DEFAULT_P2_BYTE;
	aCmdBuf[bCmdLen++] = ((PHHAL_HW_SAMAV3_ISO7816_DEFAULT_LC_BYTE + bPcdCapsInLen + bDivInputLen + 5U /* KeyNo, KeyVer, BNR, PcdCapInLen */) & 0xFF);
...
phStatus_t phhalHw_SamAV3_Cmd_MFP_AuthSectorSwitch(phhalHw_SamAV3_DataParams_t * pDataParams, uint16_t wOption, uint16_t wSSKeyBNr, uint8_t bSSKeyNo,
	uint8_t bSSKeyVer, uint8_t bMSKeyNo, uint8_t bMSKeyVer, uint8_t bSectorCount, uint8_t * pKeyBlocks, uint8_t bKeyBlocksLen,  uint8_t * pDivInput,
	uint8_t bDivInputLen, uint8_t * pPiccReturnCode)
{
	phStatus_t	PH_MEMLOC_REM wStatus = 0;
	uint8_t		PH_MEMLOC_REM aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH + 7 /* SSKeyBNr, SSKeyNo, SSKeyVer, MsKeyNo, MSKeyVer, SectorCount */];
	uint8_t		PH_MEMLOC_REM bCmdLen = 0;
	uint8_t *	PH_MEMLOC_REM pResponse = NULL;
	uint16_t	PH_MEMLOC_REM wRespLen = 0;
		...
		((wOption & PH_EXCHANGE_BUFFER_MASK) == PH_EXCHANGE_DEFAULT))
	{
		...
		/* Add Master Key number and version if set in P1.*/
		if((wOption & PHHAL_HW_SAMAV3_MFP_SSAUTH_MASTER_SECTOR_DIV_ON) == PHHAL_HW_SAMAV3_MFP_SSAUTH_MASTER_SECTOR_DIV_ON)
		{
			aCmdBuf[bCmdLen++] = bMSKeyNo;
			aCmdBuf[bCmdLen++] = bMSKeyVer;
		}
	...
	if ((wOption & PH_EXCHANGE_BUFFER_MASK) == PH_EXCHANGE_BUFFER_CONT)
	{
		...
		if((wOption & PHHAL_HW_SAMAV3_MFP_SSAUTH_BUFFER_KEY_BLOCKS) == PHHAL_HW_SAMAV3_MFP_SSAUTH_BUFFER_KEY_BLOCKS)
		{
			PH_CHECK_SUCCESS_FCT(wStatus, phhalHw_SamAV3_Cmd_7816Exchange(
				pDataParams,
				PH_EXCHANGE_BUFFER_CONT,
		...
		if((wOption & PHHAL_HW_SAMAV3_MFP_SSAUTH_BUFFER_DIV_INPUT) == PHHAL_HW_SAMAV3_MFP_SSAUTH_BUFFER_DIV_INPUT)
		{
			PH_CHECK_SUCCESS_FCT(wStatus, phhalHw_SamAV3_Cmd_7816Exchange(
				pDataParams,
				PH_EXCHANGE_BUFFER_CONT,
...
phStatus_t phhalHw_SamAV3_Cmd_PDC_Authenticate(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bOption, uint8_t bKeyNo, uint8_t bKeyVer,
	uint16_t wUpgradeKey, uint8_t * pUpgradeInfo, uint8_t bLen, uint8_t * pDivInput, uint8_t bDivInputLen, uint8_t * pPiccReturnCode)
{
	phStatus_t	PH_MEMLOC_REM wStatus = 0;
	uint8_t		PH_MEMLOC_REM aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH + 5 /* KeyNo, KeyVer, UpgradeKey, Len */];
	uint8_t		PH_MEMLOC_REM bCmdLen = 0;
	uint8_t *	PH_MEMLOC_REM pResponse = NULL;
	uint16_t	PH_MEMLOC_REM wRespLen = 0;
	...
	aCmdBuf[bCmdLen++] = bOption;
	aCmdBuf[bCmdLen++] = PHHAL_HW_SAMAV3_ISO7816_DEFAULT_P2_BYTE;
	aCmdBuf[bCmdLen++] = ((5 /* KeyNo, KeyVer, UpgradeKey, Len */ + bLen + bDivInputLen) & 0xFF);
...
phStatus_t phhalHw_SamAV3_Cmd_MFP_CombinedRead(phhalHw_SamAV3_DataParams_t * pDataParams,  uint16_t wOption, uint8_t * pReadCmd,
...
phStatus_t phhalHw_SamAV3_Cmd_MFP_CombinedWrite(phhalHw_SamAV3_DataParams_t * pDataParams, uint16_t wOption, uint8_t * pData, uint8_t bDataLen,
...
phStatus_t phhalHw_SamAV3_Cmd_MFP_ChangeKey(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bOption, uint8_t bCmdCode, uint16_t wBlockNo,
	uint8_t bKeyNo, uint8_t bKeyVer, uint8_t * pDivInput, uint8_t bDivInputLen, uint8_t * pPiccReturnCode)
{
	phStatus_t	PH_MEMLOC_REM wStatus = 0;
	uint8_t		PH_MEMLOC_REM aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH + 5 /* CmdCode, BlockNo, KeyNo, KeyVer */];
	uint8_t		PH_MEMLOC_REM bCmdLen = 0;
	uint8_t *	PH_MEMLOC_REM pResponse = NULL;
	uint16_t	PH_MEMLOC_REM wRespLen = 0;
	...
	aCmdBuf[bCmdLen++] = PHHAL_HW_SAMAV3_ISO7816_DEFAULT_P2_BYTE;
	aCmdBuf[bCmdLen++] = ((5U /* CmdCode, BlockNo, KeyNo, KeyVer */ + bDivInputLen) & 0xFF);
...
phStatus_t phhalHw_SamAV3_Cmd_MFP_WritePerso(phhalHw_SamAV3_DataParams_t * pDataParams, uint16_t wOption, uint8_t * pBlocks, uint8_t bBlocksLen,
	uint8_t * pPiccReturnCode)
{
	phStatus_t  PH_MEMLOC_REM wStatus = 0;
	uint8_t     PH_MEMLOC_REM aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH];
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_AuthenticateMIFARE_Part1(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bOption, uint8_t * pMFUID,
	uint8_t bKeyNo, uint8_t bKeyVer, uint8_t bKeyType, uint8_t bMFBlockNo, uint8_t bDivBlockNo, uint8_t * pNumberRB, uint8_t bNumRBLen,
	uint8_t ** ppEncToken, uint16_t * pEncTokenLen)
{
	...
	bCmdLen = ((bCmdLen + bNumRBLen) & 0xFF);
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_ChangeKeyMIFARE(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bOption, uint8_t bKeyNo, uint8_t bKeyVerA,
	uint8_t bKeyVerB, uint8_t * pAccCond, uint8_t * pMFUID, uint8_t bDivBlockNo, uint8_t ** ppProtData, uint16_t * pProtDataLen)
{
	phStatus_t	PH_MEMLOC_REM wStatus = 0;
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_ChangeKeyMIFAREDump(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bOption, uint8_t bKeyNo, uint8_t bKeyVer,
	uint8_t bKeyType, uint8_t * pMFUID, uint8_t bDivBlockNo, uint8_t ** ppSecretKey, uint16_t * pSecretKeyLen)
{
	phStatus_t	PH_MEMLOC_REM wStatus = 0;
	uint8_t		PH_MEMLOC_REM aCmdBuf[13];
	uint8_t		PH_MEMLOC_REM bCmdLen = 0;
...
phStatus_t phhalHw_SamAV3_Cmd_MF_Authenticate(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bOption, uint8_t * pMFUID,
...
phStatus_t phhalHw_SamAV3_Cmd_MF_Read(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t * pBlocks, uint8_t bBlocksLen,
...
phStatus_t phhalHw_SamAV3_Cmd_MF_Write(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bOption, uint8_t * pData,
...
phStatus_t phhalHw_SamAV3_Cmd_MF_ValueWrite(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bOption, uint8_t * pBlocks,
...
phStatus_t phhalHw_SamAV3_Cmd_MF_Increment(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bOption, uint8_t * pBlocks,
    uint8_t bBlocksLen, uint8_t ** ppTMData, uint16_t * pTMDataLen)
{
    phStatus_t	PH_MEMLOC_REM wStatus = 0;
	uint8_t		PH_MEMLOC_REM aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH];
...
phStatus_t phhalHw_SamAV3_Cmd_MF_Decrement(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bOption, uint8_t * pBlocks,
    uint8_t bBlocksLen, uint8_t ** ppTMData, uint16_t * pTMDataLen)
{
    phStatus_t	PH_MEMLOC_REM wStatus = 0;
	uint8_t		PH_MEMLOC_REM aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH];
...
phStatus_t phhalHw_SamAV3_Cmd_MF_Restore(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bOption, uint8_t * pBlocks,
    uint8_t bBlocksLen, uint8_t ** ppTMData, uint16_t * pTMDataLen)
{
    phStatus_t	PH_MEMLOC_REM wStatus = 0;
	uint8_t		PH_MEMLOC_REM aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH];
...
phStatus_t phhalHw_SamAV3_Cmd_MF_AuthenticatedRead(phhalHw_SamAV3_DataParams_t * pDataParams, uint16_t wOption, uint8_t * pMFUID,
	uint8_t bCmdSettings, uint8_t bKeyNo, uint8_t bKeyVer, uint8_t bKeyType, uint8_t bAuthBlockNo, uint8_t bDivBlockNo,
	uint8_t * pBlocks, uint8_t bBlocksLen, uint8_t ** ppData, uint16_t * pDataLen)
{
	...
	if((bCmdSettings & PHHAL_HW_SAMAV3_CMD_MF_AUTH_RW_KEY_INFO_NOT_AVAILABLE) != PHHAL_HW_SAMAV3_CMD_MF_AUTH_RW_KEY_INFO_NOT_AVAILABLE)
	{
	...
	if((bCmdSettings & PHHAL_HW_SAMAV3_CMD_MF_AUTH_RW_CMD_SET_DIV_ON) == PHHAL_HW_SAMAV3_CMD_MF_AUTH_RW_CMD_SET_DIV_ON)
...
phStatus_t phhalHw_SamAV3_Cmd_MF_AuthenticatedWrite(phhalHw_SamAV3_DataParams_t * pDataParams, uint16_t wOption, uint8_t * pMFUID,
	uint8_t bCmdSettings, uint8_t bKeyNo, uint8_t bKeyVer, uint8_t bKeyType, uint8_t bAuthBlockNo, uint8_t bDivBlockNo,
...
phStatus_t phhalHw_SamAV3_Cmd_MF_ChangeKey(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bOption, uint8_t bKeyNo, uint8_t bKeyVerA,
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_PwdAuthUL_Part1(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bKeyNo, uint8_t bKeyVer,
	uint8_t * pDivInput, uint8_t bDivInputLen, uint8_t ** ppPwd, uint16_t * pPwdLen)
{
	phStatus_t	PH_MEMLOC_REM wStatus = 0;
	uint8_t		PH_MEMLOC_REM aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH + 2 /* Key No, Key version. */];
	uint8_t		PH_MEMLOC_REM bCmdLen = 0;
    ...
    aCmdBuf[bCmdLen++]	= PHHAL_HW_SAMAV3_ISO7816_DEFAULT_P2_BYTE;
	aCmdBuf[bCmdLen++]	= ((2 /* Key No, Key version. */ +  bDivInputLen) & 0xFF);
...
phStatus_t phhalHw_SamAV3_Cmd_UL_PwdAuthUL(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bKeyNo, uint8_t bKeyVer,
	uint8_t * pDivInput, uint8_t bDivInputLen, uint8_t * pStatusCode)
{
	...
	*pStatusCode = 0x00;
	...
	aCmdBuf[bCmdLen++]	= (uint8_t) (bDivInputLen ? 0x01 : PHHAL_HW_SAMAV3_ISO7816_DEFAULT_P1_BYTE);
    aCmdBuf[bCmdLen++]	= PHHAL_HW_SAMAV3_ISO7816_DEFAULT_P2_BYTE;
	aCmdBuf[bCmdLen++]	= ((2U /* Key number and version. */ +  bDivInputLen) & 0xFF);
...
phStatus_t phhalHw_SamAV3_Cmd_ULC_AuthenticatePICC(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bOption, uint8_t bKeyNo, uint8_t bKeyVer,
	uint8_t * pDivInput, uint8_t bDivInputLen, uint8_t * pStatusCode)
{
	phStatus_t	PH_MEMLOC_REM wStatus = 0;
	phStatus_t	PH_MEMLOC_REM bCmdLen = 0;
	...
	/* Parameter validation. */
	PH_ASSERT_NULL_DATA_PARAM(pDataParams, PH_COMP_HAL);
	if(bOption & PHHAL_HW_SAMAV3_CMD_ULC_AUTHENTICATE_DIV_ON) PH_ASSERT_NULL_PARAM(pDivInput, PH_COMP_HAL);
	PH_ASSERT_NULL_PARAM(pStatusCode, PH_COMP_HAL);
	...
	*pStatusCode = 0x00;
	...
	aCmdBuf[bCmdLen++]	= bOption;
    aCmdBuf[bCmdLen++]	= PHHAL_HW_SAMAV3_ISO7816_DEFAULT_P2_BYTE;
	aCmdBuf[bCmdLen++]	= ((2 /* Key number and version. */ +  bDivInputLen) & 0xFF);
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_CommitReaderID_Part1(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bState, uint16_t wBlockNr,
	uint8_t ** ppResponse, uint16_t * pRespLen)
{
	phStatus_t	PH_MEMLOC_REM wStatus = 0;
	uint8_t		PH_MEMLOC_REM bCmdLen = 0;
	uint8_t		PH_MEMLOC_REM aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH + 2 /* For framing block number */];
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_CommitReaderID_Part2(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bPiccErrCode, uint8_t * pData,
	uint8_t bDataLen, uint8_t * pPiccReturnCode)
{
	...
	aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_P1_POS]	 = PHHAL_HW_SAMAV3_ISO7816_DEFAULT_P1_BYTE;
    aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_P2_POS]	 = PHHAL_HW_SAMAV3_ISO7816_DEFAULT_P2_BYTE;
	aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_LC_POS]	 = ((bDataLen + 1U /* Picc Status code */) & 0xFF);
...
phStatus_t phhalHw_SamAV3_Cmd_TMRI_CommitReaderID(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bISOMode, uint8_t bState, uint16_t wBlockNr,
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_AuthenticateTAM1(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bOption, uint8_t bKeyNo, uint8_t bKeyVer,
	uint8_t * pData, uint8_t bDataLen, uint8_t ** ppIChallange, uint16_t * pIChallangeLen)
{
	...
	if(bOption == PHHAL_HW_SAMAV3_CMD_TAM_GET_RND)
	{
		...
		aCmdBuf[bCmdLen++] = (uint8_t) PHHAL_HW_SAMAV3_CMD_TAM_CLEAR /* Custom data flag not required for TAM1 Request.*/;
		aCmdBuf[bCmdLen++] = PHHAL_HW_SAMAV3_ISO7816_DEFAULT_P2_BYTE;
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_AuthenticateTAM2(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bOption, uint16_t wKeyNo, uint16_t wKeyVer,
	uint8_t * pData, uint8_t bDataLen, uint8_t bBlockSize, uint8_t bBlockCount, uint8_t bProtMode, uint8_t ** ppResponse, uint16_t * pResponseLen)
{
	...
	if(bOption == PHHAL_HW_SAMAV3_CMD_TAM_GET_RND)
	{
		...
		aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_P1_POS] = (((bProtMode << 4) | PHHAL_HW_SAMAV3_CMD_TAM_SET /* Custom data flag required for TAM2 Request.*/) & 0xFF);
		...
		aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_P2_POS] = PHHAL_HW_SAMAV3_ISO7816_DEFAULT_P2_BYTE;
		aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_LC_POS] = ((2U /* KeyNo + KeyVer */ + bDataLen) & 0xFF);

		bCmdLen = PHHAL_HW_SAMAV3_ISO7816_LC_POS + 1;

		aCmdBuf[bCmdLen++] = (wKeyNo & 0xFF);
		aCmdBuf[bCmdLen++] = (wKeyVer & 0xFF);
	...
	if(bOption == PHHAL_HW_SAMAV3_CMD_TAM_PROCESS_TRESPONE)
	{
		...
		if(bBlockSize)
		{
			 ...
			 */
			bCustDataLen = ((((bBlockCount / 2) + (bBlockCount % 2)) * 16) & 0xFF);
		}
		else
		{
			/*
			 ...
			 * Finally mutiplying by 16 because the CustomData is represented as D * 128 => D * 16
			 */
			bCustDataLen = (((((bBlockCount - 1) + 8) / 8) * 16) & 0xFF);
		}
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_AuthenticateMAM1(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bKeyNo, uint8_t bKeyVer, uint8_t * pData,
	uint8_t bDataLen, uint8_t bPurposeMAM2, uint8_t ** ppIChallange, uint16_t * pIChallangeLen)
{
	...
	/* Validate the parameters. */
	PH_ASSERT_NULL_DATA_PARAM(pDataParams, PH_COMP_HAL);
	if(bDataLen) PH_ASSERT_NULL_DATA_PARAM(pData, PH_COMP_HAL);
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_AuthenticateMAM2(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t * pData, uint8_t bDataLen, uint8_t ** ppIResponse,
...
phStatus_t phhalHw_SamAV3_Cmd_EMVCo_RecoverStaticData( phhalHw_SamAV3_DataParams_t * pDataParams, uint16_t wOption, uint8_t * pSignedStaticAppData,
...
phStatus_t phhalHw_SamAV3_Cmd_EMVCo_RecoverDynamicData( phhalHw_SamAV3_DataParams_t * pDataParams, uint16_t wOption, uint8_t * pSignedDynamicAppData,
...
phStatus_t phhalHw_SamAV3_Cmd_EMVCo_EncipherPin(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t * pPinBlock, uint8_t * pIccNum,
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_PLExec(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bLFI, uint8_t * pPLData,
...
phStatus_t phhalHw_SamAV3_Cmd_SAM_PLUpload(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bIsFirstFrame, uint8_t bIsFinalFrame,
	uint16_t wUploadCtr, uint8_t bKeyNo, uint8_t bKeyVer, uint8_t * pPLCode, uint16_t wPLCodeLen, uint8_t * pPLReKey, uint8_t bPLReKeyLen)
{
	phStatus_t	PH_MEMLOC_REM wStatus = 0;
	...
	if(!bIsFinalFrame) PH_ASSERT_NULL_PARAM(pPLReKey, PH_COMP_HAL);
	...
	if(bIsFirstFrame)
	{
		...
		memcpy(pDataParams->aPLUploadSessMAC0, aSessionKey, ((bKeyType == PH_CRYPTOSYM_KEY_TYPE_AES128) ? 16 :
	...
	for(wIteration = 0; wIteration < wPLCodeChunks; wIteration++)
	{
		...
		if((wIteration + 1) == wPLCodeChunks)
		{
			...
			wPaddOutLen = ((wRemPLCodeLen + (16U - wRemPLCodeLen % 16U) & 0xFF) & 0xFFFF);
		...
		memcpy(&pDataParams->pPLUploadBuf[pDataParams->wPLUploadBufLen], &aCmdBuf[bCmdBuffOffset], bCmdLen - bCmdBuffOffset);
...
phStatus_t phhalHw_SamAV3_Cmd_RC_RFControl(phhalHw_SamAV3_DataParams_t * pDataParams, uint16_t wTime)
{
	...
	aCmdBuf[bCmdLen++] = (wTime & 0xFF);
	aCmdBuf[bCmdLen++] = ((wTime >> 8) & 0xFF);
...
{
	phStatus_t	PH_MEMLOC_REM wStatus = 0;
	uint8_t		PH_MEMLOC_REM aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH + 2 /* Command Code, LE byte */];
	uint8_t		PH_MEMLOC_REM bCmdLen = 0;
	uint8_t *	PH_MEMLOC_REM pResponse = NULL;
	uint16_t	PH_MEMLOC_REM wRespLen = 0;
...
phStatus_t phhalHw_SamAV3_Cmd_X_ISO14443_3_AnticollisionSelect(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t * pSelCodes, uint8_t bSelCodesLen,
...
phStatus_t phhalHw_SamAV3_Cmd_X_ISO14443_3_ActivateIdle(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bOption, uint8_t bNumCards, uint16_t wTime,
	uint8_t * pAtqaIn, uint8_t * pSakIn, uint8_t ** ppResponse, uint16_t * pRespLen)
{
	...
	/* Parameter validation. */
	PH_ASSERT_NULL_DATA_PARAM(pDataParams, PH_COMP_HAL);
	if (bOption & PHHAL_HW_SAMAV3_CMD_ISO14443_3_ACTIVATEIDLE_APPLY_ATQA) PH_ASSERT_NULL_PARAM(pAtqaIn, PH_COMP_HAL);
	if (bOption & PHHAL_HW_SAMAV3_CMD_ISO14443_3_ACTIVATEIDLE_APPLY_SAK) PH_ASSERT_NULL_PARAM(pSakIn, PH_COMP_HAL);
	PH_ASSERT_NULL_PARAM(pRespLen, PH_COMP_HAL);
...
phStatus_t phhalHw_SamAV3_Cmd_X_ISO14443_3_TransparentExchange(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t * pTxBuf, uint8_t bTxLen,
	uint8_t bTxBitLen, uint8_t ** ppRxBuf, uint16_t * pRxLen, uint8_t * pRxBitLen)
{
	...
	*pRxLen = 0;
	...
	aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_CLA_POS] = PHHAL_HW_SAMAV3_ISO7816_CLA_BYTE;
	aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_INS_POS] = PHHAL_HW_SAMAV3_CMD_ISO14443_3_TRANSPARENT_EXCHANGE_INS;
...
phStatus_t phhalHw_SamAV3_Cmd_X_ISO14443_4_RATS_PPS(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bCidIn, uint8_t bDsiIn, uint8_t bDriIn,
    uint8_t * pCidOut, uint8_t * pDsiOut, uint8_t * pDriOut, uint8_t * pAts)
{
	...
	*pDsiOut = pResponse[2];
    ...
    if((wRespLen - (uint8_t)3U) > 0)
    {
        memcpy(pAts, &pResponse[3], wRespLen - 3U);  /* PRQA S 3200 */
    }

	return PH_ADD_COMPCODE(PH_ERR_SUCCESS, PH_COMP_HAL);
}

phStatus_t phhalHw_SamAV3_Cmd_X_ISO14443_4_Init(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bCid, uint8_t bDri, uint8_t bDsi, uint8_t bFwi,
	uint8_t bFsci)
{
	phStatus_t	PH_MEMLOC_REM wStatus = 0;
	uint8_t		PH_MEMLOC_REM aCmdBuf[PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH + 5 /* CID, DRI, DSI, FWI, FSCI */];
	uint8_t		PH_MEMLOC_REM bCmdLen = 0;
	uint8_t *	PH_MEMLOC_REM pResponse = NULL;
	uint16_t	PH_MEMLOC_REM wRespLen = 0;
...
phStatus_t phhalHw_SamAV3_Cmd_X_ISO14443_4_Exchange(phhalHw_SamAV3_DataParams_t * pDataParams, uint16_t wOption, uint8_t * pAppDataIn,
