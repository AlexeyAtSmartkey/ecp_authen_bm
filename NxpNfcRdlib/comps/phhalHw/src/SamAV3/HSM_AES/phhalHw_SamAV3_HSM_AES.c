    ...
    *pTxLength = wTxLength;
    ...
    if (bFirst)
    {
        ...
        if (bLast)
        {
            ...
            pBuffer[PHHAL_HW_SAMAV3_ISO7816_LC_POS] = ((bLc + 8U) & 0xFF);
    ...
    wHelper = (uint16_t)((16 - wValidMacData) & 0xFFFF);
    ...
    if (bLast)
    {
        ...
        else if (bLcPresent)
        {
            ...
            wTxLength = ((wTxLength + bMacLength) & 0xFFFF);
        }
        else if (bLePresent)
        {
            /* Le is still on the correct position, so copy mac and move Le */
	...
	uint8_t bLast)
{
    ...
    if (wRxLength>2 && ((pDataParams->bResponseChaining == PHHAL_HW_SAMAV3_HSM_AES_CHAINING) || (!bFirst && bLast)))
    {
        ...
        if (!bFirst && bOldPendingRespDataLength)
        {
            ...
            wRxLength = ((wRxLength + bOldPendingRespDataLength) & 0xFFFF);
        }
    ...
    *pRxLength = 0;
    ...
    {
        if(wRxLength > PHHAL_HW_SAMAV3_ISO7816_SW1SW2_LENGTH)
        {
            wPayloadLength  = (uint16_t)((wRxLength - (uint16_t)(PHHAL_HW_SAMAV3_ISO7816_SW1SW2_LENGTH)) & 0xFFFF);
        }
        else
        {
            wPayloadLength = 0;
    ...
    wHelper = (uint16_t)(((uint16_t)16U - wValidMacData) & 0xFFFF);
    if (wPayloadLength)
    {
    ...
    wValidMacData = ((wValidMacData + wHelper) & 0xFFFF);
