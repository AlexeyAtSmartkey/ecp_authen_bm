...
#include "phacDiscLoop_Sw_Int_ECP.h"
...
#ifdef NXPBUILD__PHAC_DISCLOOP_SW_ECP
                || ((bPollTech == PHAC_DISCLOOP_TECH_TYPE_VAS) &&
                ((pDataParams->bPasPollTechCfg & PHAC_DISCLOOP_POS_BIT_MASK_VAS) != PHAC_DISCLOOP_POS_BIT_MASK_VAS))
#endif /* NXPBUILD__PHAC_DISCLOOP_SW_ECP */
                )
            {
...
#ifdef NXPBUILD__PHAC_DISCLOOP_SW_ECP
            /* Only one technology(VAS) enabled */
            if((bPollTech == PHAC_DISCLOOP_TECH_TYPE_VAS) &&
                (pDataParams->bPasPollTechCfg == PHAC_DISCLOOP_POS_BIT_MASK_VAS))
            {
                wPTGT = PH_NXPNFCRDLIB_CONFIG_PRE_PTGT;
                PH_CHECK_SUCCESS_FCT(status, phacDiscLoop_Sw_SetConfig(pDataParams, PHAC_DISCLOOP_CONFIG_GTVAS_VALUE_US, wPTGT));
                return status;
            }
#endif /* NXPBUILD__PHAC_DISCLOOP_SW_ECP */
...
#ifdef NXPBUILD__PHAC_DISCLOOP_SW_ECP
            if(pDataParams->pPasTechPollSeq[bTechLoopIndex] == PHAC_DISCLOOP_TECH_TYPE_VAS)
            {
                PH_CHECK_SUCCESS_FCT(status, phacDiscLoop_Sw_SetConfig(pDataParams, PHAC_DISCLOOP_CONFIG_GTVAS_VALUE_US, wPTGT));
            }
#endif /* NXPBUILD__PHAC_DISCLOOP_SW_ECP */
