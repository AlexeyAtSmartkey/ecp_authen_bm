...
#ifdef NXPBUILD__PHAL_MFUL_SAMAV3_NONX
            if((*((uint16_t*)(pDataParams)) & 0xFF) == PHAL_MFUL_SAMAV3_NONX_ID)         /* Check component layer ID and Compute CMAC */
            {
                ...
                memcpy(aMac, pMac, wMLen);  /* PRQA S 3200 */
...
#ifdef NXPBUILD__PHAL_MFUL_SAMAV3_NONX
            if(PH_GET_COMPID(pDataParams) == PHAL_MFUL_SAMAV3_NONX_ID)         /* Verify CMAC */
            {
                /* Increment the command counter. */
                ...
                aInBuff[bInBufLen++] = (uint8_t) (((phalMful_SamAV3_NonX_DataParams_t *) pDataParams)->wCmdCtr);
                aInBuff[bInBufLen++] = (uint8_t) (((phalMful_SamAV3_NonX_DataParams_t *) pDataParams)->wCmdCtr >> 8U);
                memcpy(&aInBuff[bInBufLen], *ppResponse, (*pRespLen - 8 /* MAC */)); /* PRQA S 3200 */
