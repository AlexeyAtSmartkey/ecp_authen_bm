...
void phApp_PrintTech(uint8_t TechType)
{
    ...
    case PHAC_DISCLOOP_POS_BIT_MASK_VAS:
        DEBUG_PRINTF ("\tResolving Type VAS... \n");
        break;
...
void phApp_PrintTagInfo(phacDiscLoop_Sw_DataParams_t *pDataParams, uint16_t wNumberOfTags, uint16_t wTagsDetected)
{
    ...
    if (PHAC_DISCLOOP_CHECK_ANDMASK(wTagsDetected, PHAC_DISCLOOP_POS_BIT_MASK_VAS))
    {
        DEBUG_PRINTF("\tTechnology  : VAS");
        DEBUG_PRINTF("\n");
    }
